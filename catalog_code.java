...
public class PolynomialSolver {
    ...
    public static long decodeValue(int base, String value) {
        ...
        for (char c : value.toCharArray()) {
            ...
            decodedValue = decodedValue * base + digit;  // Accumulate the decoded value
        }
        return decodedValue;
    }
    ...
    public static double lagrangeInterpolation(List<Map.Entry<Integer, Long>> points) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            c += term;  // Accumulate the constant term
        }

        return c;  // Return the constant term without rounding
    }
    ...
    public static void processTestCases(List<Map<String, Map<String, String>>> testCases) {
        for (int index = 0; index < testCases.size(); index++) {
            ...
            for (Map.Entry<String, Map<String, String>> entry : testCase.entrySet()) {
                if (entry.getKey().equals("keys")) continue; // Skip the keys entry
                int x = Integer.parseInt(entry.getKey());  // x is the key
                int base = Integer.parseInt(entry.getValue().get("base")); // Get the base
                long y = decodeValue(base, entry.getValue().get("value"));  // Decode y
                points.add(new AbstractMap.SimpleEntry<>(x, y));  // Add the point (x, y) to the list
            }
            ...
            if (index == 1) {
                ...
                for (Map.Entry<Integer, Long> point : points) {
                    if (Math.abs(point.getValue() - secret) > 1000) {  // Simple heuristic for wrong points
